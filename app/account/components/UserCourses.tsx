...
export default function UserCourses() {   // ✅ Renamed from UserPayments
    ...
    const fetchMyLearning = async () => {
        ...
        const coursesData = enrolledCourseIds.length
            ? (await supabase
                .from("courses")
                .select("*")
                .in("id", enrolledCourseIds)).data || []
            : [];

        setCourses(coursesData);

        // Fetch enrolled internships
        ...
        <div className="p-8 space-y-12">
                    ...
                    backdrop-blur-xl">
                                    ...
                                    <CardItem translateZ="80" className="w-full mb-3">
                                        <Image
                                            ...
                                            width={600}
                                            height={400}
                                            className="h-48 sm:h-52 w-full object-cover rounded-xl ring-1 ring-indigo-200/40 dark:ring-indigo-700/40 group-hover/card:shadow-xl transition"
                                        />
                                    </CardItem>
                                    ...
                                    <div className="flex flex-wrap justify-between gap-2 text-xs text-neutral-600 dark:text-neutral-400 mb-4">
                                        ...
                                        {course.duration && (
                                            <span className="flex items-center gap-1">
                                                <Clock className="h-3 w-3 text-indigo-500" /> {formatDuration(course.duration)}
                                            </span>
                                        )}
                                    </div>
            ...
            <div>
                ...
                {loading ? (
                    ...
                    <p className="text-center text-cyan-600 font-bold">
                        <Highlighter action="underline" color="#FFA500">
                            No internships yet. Go build real skills
                        </Highlighter>
                    </p>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    backdrop-blur-xl">
                                    ...
                                    <CardItem translateZ="80" className="w-full mb-3">
                                        <Image
                                            src={internship.thumbnail_url || "https://via.placeholder.com/600x400?text=No+Image"}
                                            alt={internship.title}
                                            width={600}
                                            height={400}
                                            className="h-48 sm:h-52 w-full object-cover rounded-xl ring-1 ring-cyan-200/40 dark:ring-cyan-700/40 group-hover/card:shadow-lg transition"
                                        />
                                    </CardItem>
                                    <CardItem as="p" translateZ="60" className="text-neutral-700 dark:text-neutral-300 text-sm mb-4 line-clamp-3">
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-4">
                                        ...
                                        {internship.categories?.map((cat) => (
                                            <Badge key={cat} variant="outline" className="border-cyan-400/40 text-cyan-700 dark:text-cyan-300">
                                                {cat}
                                            </Badge>
                                        ))}
                                    </div>
                                    ...
                                    <div className="flex flex-wrap justify-between gap-2 text-xs text-neutral-600 dark:text-neutral-400 mb-4">
                                        ...
                                        {internship.duration && (
                                            <span className="flex items-center gap-1">
                                                <Clock className="h-3 w-3 text-cyan-500" /> {formatDuration(internship.duration)}
                                            </span>
                                        )}
                                        {internship.max_seats && (
                                            <span className="flex items-center gap-1">
                                                <Users className="h-3 w-3 text-blue-500" /> {internship.max_seats} seats
                                            </span>
                                        )}
                                    </div>
